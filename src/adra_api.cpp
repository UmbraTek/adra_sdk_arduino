...
int AdraApi::get_iwdg_cyc(int id, int32_t *buf) {
  this->_id(id);
  this->_send(ROSMOS_RW::R, ADRA_REG_IWDG_CYC, NULL, NULL);
  ROSMOSType *rx_data = new ROSMOSType();
  int ret = this->_pend(rx_data, ROSMOS_RW::R, ADRA_REG_IWDG_CYC, ROSMOS_TIMEOUT);
...
int AdraApi::set_iwdg_cyc(int id, uint32_t buf) {
  ...
  this->_send(ROSMOS_RW::W, ADRA_REG_IWDG_CYC, txdata, NULL);
  ROSMOSType *rx_data = new ROSMOSType();
  int ret = this->_pend(rx_data, ROSMOS_RW::W, ADRA_REG_IWDG_CYC, ROSMOS_TIMEOUT);
  delete rx_data;
  return ret;
}
...
int AdraApi::set_cpos_target(uint8_t sid, uint8_t eid, float *pos) {
  ...
  ADRA_REG_CPOS_TARGET[3] = 2 + 4 * num;
  this->_send(ROSMOS_RW::W, ADRA_REG_CPOS_TARGET, data, num * 4 + 2);
  return 0;
}
...
int AdraApi::set_ctau_target(uint8_t sid, uint8_t eid, float *tau) {
  ...
  ADRA_REG_CTAU_TARGET[3] = 2 + 4 * num;
  this->_send(ROSMOS_RW::W, ADRA_REG_CTAU_TARGET, data, num * 4 + 2);
  return 0;
}
...
int AdraApi::set_cpostau_target(uint8_t sid, uint8_t eid, float *pos, float *tau) {
  ...
  ADRA_REG_CPOSTAU_TARGET[3] = 4 * num * 2 + 2;
  this->_send(ROSMOS_RW::W, ADRA_REG_CPOSTAU_TARGET, data, 4 * num * 2 + 2);
  return 0;
}

int AdraApi::get_spostau_current(int id, int *num, float *pos, float *tau) {
  this->_id(id);
  this->_send(ROSMOS_RW::R, ADRA_REG_SPOSTAU_CURRENT, NULL, NULL);
  ROSMOSType *rx_data = new ROSMOSType();
  int ret = this->_pend(rx_data, ROSMOS_RW::R, ADRA_REG_SPOSTAU_CURRENT, ROSMOS_TIMEOUT);
...
int AdraApi::get_cpostau_current(uint8_t sid, uint8_t eid, int *num, float *pos, float *tau, int *ret) {
  int temp = 0;
  this->_id(0x55);
  const uint8_t *cmd = ADRA_REG_CPOSTAU_CURRENT;
  uint8_t tx_data[2] = {sid, eid};
  float timeout_s = 0.5;
  int tx_len = cmd[1];
  int rx_len = cmd[2];
  this->_send(ROSMOS_RW::R, ADRA_REG_CPOSTAU_CURRENT, tx_data, tx_len);
  for (int i = 0; i < eid - sid + 1; i++) {
    ROSMOSType *rx_data = new ROSMOSType();
    ret[i] = this->_pend(rx_data, ROSMOS_RW::R, ADRA_REG_CPOSTAU_CURRENT, 100000);
